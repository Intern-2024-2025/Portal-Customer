...
const getTransactionClient = async (
  ...
  try{ 
    const resposne = await TransactionAPI.getTransactionByClient(route.params.productId, currentPage)
    dataTransaction.value = resposne.data.data
    pagination.value = resposne.data.pagination
    countStatus.value = resposne.data.countStatus
  } catch (error) {
    ...
    <div class="rounded-lg border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark w-1/2">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <h3 id="totalMasuk" class="mb-2 text-title-md font-bold text-black dark:text-white">{{  countStatus.true }}</h3>
    ...
    <div class="rounded-lg border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark w-1/2">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <h3 id="totalKeluar" class="mb-2 text-title-md font-bold text-black dark:text-white">{{ countStatus.false  }}</h3>
          <p class="flex items-center gap-1 text-sm font-medium">Transactions</p>
        </div>
      </div>
    </div>
      ...
      <table class="w-full table-auto">
        ...
        <tbody> 
          ...
          <tr v-else v-for="(item, index) in dataTransaction" :key="index">
            ...
            <td class="py-5 px-4">
              <p class="text-black dark:text-white">{{ new Date(item.create).toLocaleString() }}</p>
            </td>
            </tr>
          </tbody>
        </table>
