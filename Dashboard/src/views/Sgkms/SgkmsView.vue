...
const endpointData = {
  ...
  'Random Number': {
    ...
    data: { sessionToken: '', slotId: '', length: '' },
  ...
  'Mac Generate': {
    ...
    data: { sessionToken: '', slotId: '', keyId: '', hashAlgo: 'CMAC/HMAC-SHA256/GMAC-256', macData: '' },
  ...
  'Mac Verify GMAC-256': { 
    ...
    data: { sessionToken: '', slotId: '', keyId: '', hashAlgo: 'GMAC-256', mac:'', data:'', iv: ''},
    url: `/${selectedVersion.value}/mac/verify`,
  },
  'Seal': {
    fields: [
  ...
  'Cert Sign': {
    fields: [
      { name: 'sessionToken', label: 'Session Token', placeholder: 'The session token for authentication' },
      { name: 'slotId', label: 'Slot ID', placeholder: 'The ID of the slot associated with the request' },
      { name: 'keyId', label: 'Key ID', placeholder: 'The ID of the intermediate or root certificate private key used to sign the CSR (RSA-3072, RSA4096, or ECDSA algorithm)' },
    ...
    data: { sessionToken: '', slotId: '', keyId: '', validityPeriod: '', csr: '' },
  ...
  'External Keypair Generate RSA': { 
    fields: [
      { name: 'sessionToken', label: 'Session Token', placeholder: 'The session token for authentication' },
      { name: 'slotId', label: 'Slot Id', placeholder: 'The ID of the slot associated with the request' },
      { name: 'algo', label: 'Algorithm', placeholder: 'The ID of the key used for MAC verification' },
      { name: 'algoLength', label: 'Algorithm Length', placeholder: 'The length of the key in bits(e.g., 2048, 3072, 4096)' },
  ...
  'External Encrypt Asymmetric Encryption With Session Key': {
    fields: [
      ...
      { name: 'slotId', label: 'Slot Id', placeholder: 'The ID of the slot associated with the request' },
      { name: 'publicKeyOrCert', label: 'Public Key', placeholder: 'The public key or certificate used for encryption' },
      { name: 'useSessionKey', label: 'Use Session Key', placeholder: 'Optional: Use session key for RSA encryption' },
      { name: 'text1', label: 'Plaintext text 1', placeholder: 'The plaintext data to be encrypted' },
      { name: 'text2', label: 'Plaintext text 2', placeholder: 'The plaintext data to be encrypted' },
    ],
  ...
  'External Decrypt Symmetric Decryption': {
    fields: [
      ...
      { name: 'iv1', label: 'Ciphertext iv 1', placeholder: 'Initialization Vector(IV) used during encryption' },
      ...
      { name: 'iv2', label: 'Ciphertext iv 2', placeholder: 'Initialization Vector(IV) used during encryption' },
    ],
  ...
  'External Tokenize': {
    fields: [
      ...
      { name: 'text1', label: 'Text 1', placeholder: 'The plaintext data to be tokenized' },
      { name: 'formatChar1', label: 'Format Chararcter 1', placeholder: 'Optional: Format-preserving characters (e.g., credit card masks)' },        
      { name: 'text2', label: 'Text 2', placeholder: 'The plaintext data to be tokenized' },
      { name: 'formatChar2', label: 'Format Chararcter 2', placeholder: 'Optional: Format-preserving characters (e.g., credit card masks)' },        
    ],
  ...
  'External Detokenize': {
    fields: [
      ...
      { name: 'token1', label: 'Token 1', placeholder: ' The tokenized data to be detokenized' },
      { name: 'metadata1', label: 'Meta Data 1', placeholder: '  Optional: Metadata related to tokenization' },
      { name: 'token2', label: 'Token 2', placeholder: ' The tokenized data to be detokenized' },
      { name: 'metadata2', label: 'Meta Data 2', placeholder: '  Optional: Metadata related to tokenization' },
    ],
